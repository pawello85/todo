...
func initialModel(filename string) model {
	loadedThemes := loadThemes(defaultThemesFile)
...
func (m model) renderTrash(height int, t Theme) string {
	...
	for i := start; i < end; i++ {
		...
		// --- TREE DRAWING FOR TRASH (Now using Full Logic!) ---
		treePrefix := ""
		if item.level == 0 {
			treePrefix = " "
		} else {
		...
		// Trash Item (Align [D] with [ ] - both 3 chars)
...
func loadThemes(path string) []Theme {
